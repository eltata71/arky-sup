...
-- Sesiones vivas. En Supabase Auth, revocar es borrar la fila.
...
do $$ declare n int; r boolean; begin
  set local role authenticated;
  perform set_config('request.jwt.claims', '{"sub":"40000000-0000-4000-8000-000000000005","role":"authenticated","session_id":"60000000-0000-4000-8000-000000000005"}', true);
  select count(*) into n from api.user_profiles;
  select exists (select 1 from api.current_permissions()) into r;
  reset role;
...
do $$ declare n int; begin
  set local role authenticated;
  perform set_config('request.jwt.claims', '{"sub":"40000000-0000-4000-8000-000000000002","role":"authenticated","session_id":"60000000-0000-4000-8000-000000000002"}', true);
  select count(*) into n from api.user_profiles;
  reset role;
...
do $$ declare m text; begin
  begin
    set local role authenticated;
    perform set_config('request.jwt.claims', '{"sub":"40000000-0000-4000-8000-000000000002","role":"authenticated","session_id":"60000000-0000-4000-8000-000000000002"}', true);
    perform api.set_user_role('40000000-0000-4000-8000-000000000003', 'trainer');
    reset role;
    m := 'FALLO: permitido';
  exception when others then m := 'OK: ' || sqlstate; end;
  insert into probe_results values ('20 sesión revocada no cambia roles', m);
end $$;
...
do $$ declare m text; begin
  begin
    ...
    m := 'FALLO: permitido';
  exception when others then m := 'OK: ' || sqlstate; end;
  insert into probe_results values ('21 token sin session_id rechazado', m);
end $$;

do $$ declare m text; begin
  begin
    set local role authenticated;
    perform set_config('request.jwt.claims', '{"sub":"40000000-0000-4000-8000-000000000001","role":"authenticated","session_id":"60000000-0000-4000-8000-000000000004"}', true);
    perform api.set_user_role('40000000-0000-4000-8000-000000000003', 'trainer');
    reset role;
    m := 'FALLO: permitido';
  exception when others then m := 'OK: ' || sqlstate; end;
  insert into probe_results values ('22 sesión de otro usuario rechazada', m);
end $$;
...
do $$ declare m text; begin
  begin
    ...
    m := 'FALLO: permitido';
  exception when others then m := 'OK: ' || sqlstate; end;
  insert into probe_results values ('23 sesión pasada de fecha techo rechazada', m);
end $$;
update auth.sessions set not_after = null
  where id = '60000000-0000-4000-8000-000000000001';

-- Caso 24: la auditoría registró exactamente los cambios permitidos.
do $$ declare n int; begin
  select count(*) into n from private.authorization_audit;
  insert into probe_results values ('24 auditoría con 2 entradas', case when n = 2 then 'OK' else 'FALLO: ' || n end);
end $$;
