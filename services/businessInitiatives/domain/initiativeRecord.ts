/**
 * Cómo se construye y cómo se lee una Iniciativa, sin tocar la base de datos.
 *
 * Estaba dentro del repositorio (F3-05): el normalizador y la construcción son
 * reglas del modelo —qué es una iniciativa utilizable, con qué valores por
 * defecto nace— y el repositorio sólo necesita llamarlas. Separadas, se prueban
 * sin mock alguno y el repositorio queda en lo que es: la puerta a la
 * persistencia.
 *
 * El normalizador es generoso con lo que acepta y estricto con lo que devuelve:
 * una iniciativa guardada con un esquema anterior, o editada a mano en SQL,
 * carga con cada campo en un valor utilizable en vez de tumbar el portafolio.
 */
...
import { toInitiativeRevision } from './initiativeRevision';
...
export const normalizeInitiative = (
  ...
  return {
    ...
    revision: toInitiativeRevision(raw.revision),
  };
};
