...
- **Tarea en curso:** **F6-03, corte 3 — la Oficina con la forma del
  ...
  - Cinco reglas llegaban a la base por un solo import: `withAuditEntry` y los
    generadores de ids vivían en el repositorio. Ahora están en
    `domain/officeEngagementRecord.ts`.
  - El dominio entra en `strict` al bajar el puerto `AgentPersonaBriefing` a
    su hoja.
  - La Oficina entra en `contextDomainPurity`.
  - 4 793 pruebas; carga inicial 310,9 KB gz.

  Falta el corte 4 (Artefactos).
- **Antes:** **F6-03, corte 2b — Proyectos cambia por operaciones
...
- **Antes:** **F6-03, corte 2a — Proyectos con la forma del piloto.**
  ...
  `contextDomainPurity.test.ts` sigue el cierre de imports de valor de cada
  dominio con el analizador de TypeScript. Se comprobó con un caso negativo
  real. Todo el dominio de Proyectos entra en `strict`. Sin cambio de
  comportamiento: carga inicial 310,1 KB gz, 4 764 pruebas. Siguen: 2b
  (operaciones con nombre en lugar de `updateProject(parcial)`), 3 (Oficina)
  y 4 (Artefactos).
- **Antes:** **F6-10**, el despliegue comprueba el esquema de
...
- **Antes:** **F6-01 cerrada con el tercer corte**: el motor sólo
  ...
  Siguiente: **F6-04**.
- **Antes:** **F6-01, segundo corte — el proxy heredado.**
...
- **Antes:** **F5-04 y F5-05 — cierran la fase 5** (cierre en
  ...
  reconstrucción de un grafo existente se rechazaba en silencio (#73).
  **Migración `20260924120000_projection_outbox` aplicada a `ArkyDB-US`** el
  2026-09-24 con aprobación, antes de fusionar: historia remota alineada y las
  tres RPC verificadas en la API de producción. **La fase 5 está cerrada en el
  repositorio y en producción.** Evidencia en `evidencias/f5-04-f5-05.md`.
...
- **Antes:** **F5-03 — cerrada.** El
  ...
  architectureOffice`, `chat -> ai`). Los cuatro objetivos de fase 5 de
  `budgetTargets.mjs` están cumplidos. Evidencia en `evidencias/f5-03.md`.
  Antes: **F5-02 — cerrada** (PR #71). Las seis pantallas sobre el
...
- **Última tarea integrada:** **F5-01, corte 10** (#67, `78b92ae`) — crítica y
  ...
  `evidencias/f5-01-corte-10.md`. Antes: **F5-01, corte 9** (#66, `1195ffc`) —
  la propuesta de nombres iniciales de artefactos sale del motor y usa
  `aiGateway`; el motor queda en 2 431 líneas. Evidencia en
  `evidencias/f5-01-corte-9.md`. Antes: **F5-01, corte 8** (#64, `d35b479`) — el asistente con persona
...
- **Estado medido con F5-03 (2026-09-24):** 483 ficheros y 4 701 pruebas en
  ...
  conexo (el de React)**; **0 módulos de dominio mutuamente alcanzables**;
  0 pares ascendentes; **44 pares con import profundo**; 0 pantallas sobre el
  fan-out; `any` 7; 0 ficheros sueltos.
...
| **F5-04** outbox transaccional | ✅ (2026-09-24; #73, migración aplicada) | `api.projection_outbox` + disparador en la transacción del artefacto + `list_pending_projections`, `save_graph_projection` (guardar y marcar atómico: reprocesar no duplica, lo viejo no pisa lo nuevo) y `fail_projection`. Contrato `projection_outbox.test.sql`. Cierra H11. |
