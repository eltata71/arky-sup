...
### F6-03 · Aplicar el patrón al resto de contextos · P1 · XL · `en curso` — corte 1 (revisiones de `settings` y `learning`); corte 2a (Proyectos con `domain/` + `infrastructure/` + `application/` y pureza por cierre); **corte 2b: Proyectos cambia por operaciones con nombre** (`applyProjectCommand`, `runProjectCommand`; el grafo por su propio camino) — `evidencias/f6-03-corte-2a-proyectos.md`, `evidencias/f6-03-corte-2b-proyectos.md`. Faltan: 3 (Oficina) y 4 (Artefactos)
...
- **Origen.** Fases 4 y 5: el CI publica la aplicación y no el esquema; tres
  ...
  producción (F4-06).
- **Aceptación.** El job `deploy` se niega a publicar un commit cuyo
  `supabase/migrations/` tenga versiones que `ArkyDB-US` no tiene aplicadas.
- **Decisión pendiente.** Cómo lee el despliegue las versiones aplicadas: hoy el
  CI no tiene credenciales de Supabase y el rol anónimo no accede al esquema `api`.
