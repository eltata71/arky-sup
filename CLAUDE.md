...
| `npm run check:any-budget` | **18** `any` types, budget 18 (eran 38, y 23 hasta F5-01: el transporte salió del motor tipado) |
...
`__tests__/services/ai/engineImporters.test.ts` lists the seven that still
import the engine; the list may only shrink, and each one leaves the way the LMS
...
- **Avoid `any`.** ESLint's `no-explicit-any` is `off` globally only because the legacy monolith would produce thousands of violations; it is `error` for `lib/security.ts` and `lib/ids.ts`, and new modules should be written to that bar. Prefer precise types or `unknown` + type guards. `npm run check:any-budget` holds the repository to **18**, and what remains is named: 11 in `services/geminiService.ts`, 6 in the untyped Excalidraw boundary, and React's own `ComponentType<any>` in `lazyWithRetry`. Code that leaves the engine leaves typed — the transport went out with `unknown` and the SDK's own parameter types rather than carrying its five `any` along (F5-01).
