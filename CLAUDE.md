...
Two changes, and the second is the more interesting one. `projectWrites`
entered by file path — until F5-03 moved the model-calling compaction out of
`services/chat` into `services/ai/generation/chatCompaction.ts`, after which the
chat barrel carries no AI and `projectWrites` enters by the door again. And `deterministicCompactionDigest` — a pure, AI-less
...
| **Alcanzabilidad** | un grupo de módulos que puede volver a sí mismo siguiendo imports, aunque ningún par se importe mutuamente. `ALLOWED_SCCS` registra **sólo el de React** desde F5-03 —el de dominio pasó de 13 a 0— y **sólo puede encoger**: un componente que contenga un módulo de `services/` falla sin presupuesto — ADR-104 |
...
JSX branches. The fix for a screen over the default is to move its
orchestration into an application service the screen calls once, not to raise
an entry — and **since F5-02 there is no entry to raise**: the table in
`scripts/checkModuleBoundaries.mjs` is empty, so a third service module in a
screen fails outright.

**F5-02 took the last six screens out, and each showed which of three things it
needed** (`docs/ddd-transformacion/evidencias/f5-02.md`):

1. **A rule written in a screen goes to the context that owns it.** The copilot's
   turn routing is `routeCopilotTurn` in the Office (which drives the agent, never
   the reverse); what a `modifyArtifact` call means is
   `interpretArtifactModification` in `services/agent`; an attention's initiative
   links are `resolveAttentionInitiativeLinks` in `services/portfolioGraph`.
2. **Two contexts a screen needs at once meet in a hook** — `useCopilotTurns`,
   `useAssistantTurns`, `useInitiativeBoard`, `useInitiativeAssistant`,
   `useAttentionPortfolio`. The hook decides nothing.
3. **A type from state a context already hands over comes from that context** —
   `BusinessInitiative` from `InitiativeContext`, `OfficeEngagement` from
   `OfficeContext`, `ChatMessage` from `AppContext`, the agent vocabulary from
   `useAgentActions`. `EngagementIntakeWizard` was over the limit on two type
   declarations alone. This is the `Project` pattern, not a way round the count:
   it is only honest when the context really is the one handing the data over.
...
| `services/architectureOffice/application/copilotTurn` | **F5-02.** Where a copilot turn goes — orchestration, a plan, disambiguation or consultation — and the synthetic anchor for global actions. `ProjectCopilotChatModal` went 4 → 1 service modules |
| `services/architectureOffice/application/officeCapabilities` | **F5-02.** The capabilities panel's read model: gates with their labels and the Office's four counts |
| `services/agent/artifactModificationCall` | **F5-02.** What a `modifyArtifact` call means, and what the answer says — the guardrails that stop the assistant announcing an empty or identical change |
...
provider — the serverless proxy, retries, the timeout and model fallback. The
second job is `services/ai/generation/legacyTransport.ts` now, moved as-is and
used by the engine through delegation, with the Gemini client injected so the
engine's `getAIClient` is still the one seam tests replace. `aiGateway` and the
four façades that only send their own prompt — assisted capture, the platform
...
**The main generation got the same treatment in the thirteenth cut.** Its
prompt opens with an Office persona, and the engine used to look it up in
`services/architectureOffice`. Now it declares `ArtifactPersonaComposer`
(`lib/artifacts`), the Office supplies `composeArtifactPersonaInstruction`, the
agent derives one from its briefing (`personaComposer`), and screens get it
from `hooks/useArtifactPersona`. Every caller of `generateArtifactContent`
hands one in; without it the base instruction goes unchanged.
...
proxy — a production build, whose keys are server-side, never reached a model
there and always answered with the local ranker — and it skipped the input
guardrail and provider routing too. Out of the engine it goes through
`aiGateway` with its own budget (35 s, two retries, one model). A helper both
...
keeps it out. Do not add LMS generation to the monolith again. Its last
straggler, `evaluateChallenge`, left in F5-01's second cut
(`learning/challengeEvaluation.ts`) and left typed: the engine returned
`Promise<any>` and the façade asserted a shape nobody checked, while
...
- `@google/genai` still appears in the engine (the `GoogleGenAI` its transport receives) as well as `providers/gemini/` and `api/`. The client factory *has* moved: it is `services/ai/providers/gemini/geminiClient.ts` now.
  ...
  **Moving the engine into `services/ai/` was tried in Ola 5 and reverted, and
  ...
  services/ai` closed it on its own.

  **F5-03 dissolved it, 13 → 0, by cutting three imports.** In the layer order
  the domain already had — diagram, quality, compiler, export, knowledge graph,
  publication, project, context graph, AI, chat, artifacts, agent, Office —
  only three edges pointed up: the diagram signal extractor took a whole
  `Project` to read three fields (it now declares `DiagramSignalSource`), the
  knowledge graph fetched the Office's standards through an option no
  production caller set (`includeOfficeContext`, removed), and chat compaction
  called a model from inside `services/chat` (now
  `services/ai/generation/chatCompaction.ts`, with `CompactionDigest` in
  `lib/conversationDigest.ts`). **What decides how hard a component is to
  break is how many edges contradict the order, not how many edges it has.**
  See `docs/ddd-transformacion/evidencias/f5-03.md`.
