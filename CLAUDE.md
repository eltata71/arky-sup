...
| `npm run check:any-budget` | **17** `any` types, budget 17 (eran 38; 23 hasta F5-01, cuyo corte 1 sacó el transporte tipado y el corte 2 `evaluateChallenge`) |
...
- **Avoid `any`.** ESLint's `no-explicit-any` is `off` globally only because the legacy monolith would produce thousands of violations; it is `error` for `lib/security.ts` and `lib/ids.ts`, and new modules should be written to that bar. Prefer precise types or `unknown` + type guards. `npm run check:any-budget` holds the repository to **17**, and what remains is named: 10 in `services/geminiService.ts`, 6 in the untyped Excalidraw boundary, and React's own `ComponentType<any>` in `lazyWithRetry`. Code that leaves the engine leaves typed — the transport went out with `unknown` and the SDK's own parameter types rather than carrying its five `any` along (F5-01).
