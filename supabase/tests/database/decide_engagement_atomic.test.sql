...
-- Un encargo esperando al comité, en revisión 1.
...
select throws_ok($$select api.decide_engagement('proj_arb_001', $j${
  ...
  '42501', 'La decisión debe ir firmada por la sesión actual',
  'Nadie firma en nombre de otro');
...
select is(
  (select (data -> 'arbDecisions' -> 0) ->> 'id' from api.office_engagements where id = 'eng_arb_001'),
  'arb_001', 'El espejo contiene la decisión real, no la que envió el cliente');
